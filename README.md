...
2. `/settings`에서 본인의 Confluence Personal Access Token(PAT)만 입력 →
   ...
   없습니다(위 참고).
3. `/analyze`에서 페이지 ID(들)를 입력하고 "문서 조회" → Confluence에서
   원본만 빠르게 가져와 먼저 보여줍니다(AI 분석 없이). 원본을 확인한 뒤
   "AI 분석 실행" 버튼을 누르면 그때 규칙 검사 + LLM 심층 검토가 돌아갑니다
   - 문서 길이/LLM 응답 속도에 따라 시간이 걸릴 수 있는 단계라 별도
     버튼으로 분리했습니다. 분석 결과는 화면에 바로 렌더링되고,
     "Markdown으로 다운로드" 버튼으로 파일도 받을 수 있고, "이메일로
     받기" 버튼으로 리포트를 메일로도 받을 수 있습니다(메일 API 설정
     필요, 아래 참고). 별도 입력 없이 로그인 아이디 그대로
     `<아이디>@samsung.com`으로 발송됩니다. 각 페이지 섹션에는 원본
     Confluence 페이지로 바로 이동할 수 있는 링크가 포함됩니다.
