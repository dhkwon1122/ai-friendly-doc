...
`LLM_MAX_OUTPUT_TOKENS`를 따로 지정하지 않으면 수정본 호출의 토큰 예산은
입력 문서 길이에 맞춰 자동으로 늘어나므로(최소 4096) 보통은 신경 쓸
필요가 없습니다. **이 값을 무작정 크게(예: 수십만 이상) 잡는다고 더
안전해지지 않습니다** - 모델이 실제로 처리할 수 있는 최대 컨텍스트
길이(vLLM이면 `--max-model-len` 등으로 기동 시 정해짐)보다 크게 요청하면,
대부분의 OpenAI 호환 서버는 응답을 만들다 자르는 게 아니라 **요청 자체를
즉시 거부**합니다(그러면 안내 문구에 "This model's maximum context length
is N tokens..." 같은 메시지가 뜹니다). 이 경우엔 `LLM_MAX_OUTPUT_TOKENS`를
