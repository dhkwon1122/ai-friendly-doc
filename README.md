...
**문단 안 줄바꿈**: 표준 마크다운은 빈 줄로 안 나뉜 문단 안의 순수
개행 하나를 줄바꿈이 아니라 그냥 공백으로 접어버립니다(소프트 브레이크).
그래서 원본에 있던 줄바꿈이 그대로 전달하면 사라질 수 있어서, 그런
줄 끝에는 마크다운 하드 브레이크 문법(줄 끝 `\`)을 자동으로 붙입니다.
제목/목록/표처럼 이미 자기 줄에서 독립된 블록으로 인식되는 줄은 건드리지
않습니다.

**표 구분선**: 마크다운 표는 헤더 행 바로 다음 줄에 `|---|---|`처럼
컬럼 수만큼 구분선이 없으면 표로 인식되지 않고 그냥 파이프가 섞인
텍스트로 취급됩니다(그러면 줄 사이 개행도 소프트 브레이크로 접혀서
표 전체가 한 줄로 뭉개져 보입니다). 그래서 원본 문서의 표를 평문으로
바꿀 때 이 구분선을 항상 자동으로 넣고, LLM에게도 수정본을 쓸 때 표마다
이 구분선을 넣도록 지시합니다.
