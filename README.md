...
(`revised_document`)도 같은 LLM 호출에서 함께 만듭니다. 웹 UI의 `/analyze`
분석 결과 화면에는 페이지별로 펼칠 수 있는 아코디언이 나오고, 각 페이지를
펼치면 원본 문서와 수정본이 나란히 표시됩니다. 여러 페이지/스페이스를 한
번에 분석해도 페이지별로 아코디언을 따로 펼쳐서 비교할 수 있습니다.
`LLM_BASE_URL`이 설정 안 돼 있거나 LLM 호출이 실패한 경우에는 원본은
그대로 보여주고 수정본 자리에는 안내 문구가 대신 표시됩니다 (수정본은
LLM 없이는 만들 수 없기 때문입니다).

수정본은 문서 전체를 거의 그대로 다시 써야 해서 응답 길이가 문서 길이에
비례합니다. `LLM_MAX_OUTPUT_TOKENS`를 따로 지정하지 않으면 입력 문서
길이에 맞춰 자동으로 넉넉하게 늘어나므로(최소 4096) 보통은 신경 쓸
필요가 없습니다. 그래도 응답이 최대 토큰 제한으로 중간에 잘리면(리포트에
"최대 토큰 제한으로 중간에 잘렸습니다" 오류로 표시됨) `LLM_MAX_OUTPUT_TOKENS`를
직접 늘려서 지정하거나 `LLM_MAX_INPUT_CHARS`를 줄여보세요.
