...
않습니다). 그래서 복사용 소스 바로 위에 `<원본 제목> (AI 개선 제안)`
형식의 **제안 제목**을 따로 안내해뒀습니다 - 새 페이지 만들 때 제목
입력란에 그걸 직접 입력하면 됩니다.

**원본 문서로 돌아갈 수 있는 링크**는 복사용 소스 맨 위에 실제
Confluence storage 문단(`<p><em>원본 문서: <a href="...">...</a></em></p>`)
으로 포함돼 있습니다 - 그래서 붙여넣으면 새로 만든 페이지 본문에도 그
링크가 그대로 남습니다. 이 링크의 base URL은 REST API 호출에 쓰는
