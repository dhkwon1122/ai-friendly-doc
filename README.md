...
수정안)는 그대로 리포트에 남고, 수정본만 만들어지지 않습니다(위 안내
문구로 표시). `LLM_MAX_OUTPUT_TOKENS`를 따로 지정하지 않으면 수정본 호출의
토큰 예산은 입력 문서 길이에 맞춰 자동으로 늘어나므로(최소 4096) 보통은
신경 쓸 필요가 없습니다. 그래도 계속 잘리면(수정본이 계속 안내 문구로만
나오는 경우) `LLM_MAX_OUTPUT_TOKENS`를 직접 늘려서 지정하거나
`LLM_MAX_INPUT_CHARS`를 줄여보세요 - 그래도 안 되면 모델 자체의 최대
컨텍스트 길이(vLLM의 `--max-model-len` 등)가 문서를 통째로 왕복하기에
부족한 것일 수 있습니다.
