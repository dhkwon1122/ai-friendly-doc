...
보던 요약 표/가이드라인 준수 표/발견된 문제 목록이 이어집니다. 이 블록은
LLM이 만든 평문 수정본(제목 `#`, 목록 `-`, 표 `| a | b |`)을
`confluence_markdown_macro.py`가 Confluence 내장 **"Markdown" 매크로**
(`<ac:structured-macro ac:name="markdown">`) XML로 감싼 것입니다. 사용법:
메일에서 이 블록을 전체 선택해서 복사 → Confluence에서 새 페이지
만들기 → 편집기 오른쪽 위 **⋯(더보기) 메뉴 → 소스 편집기**를 열어 그대로
붙여넣기 → 서식이 적용된 페이지 완성.

**소스 편집기는 페이지 본문만 편집할 수 있고 제목은 별도 입력란입니다**
(마크다운에 `#`으로 제목을 넣어도 그건 본문 안 제목일 뿐 페이지 제목이
되지 않습니다). 새 페이지 제목은 **원본 문서 제목을 그대로** 입력하면
됩니다 - 별도로 다른 제목을 제안하지 않습니다.

**원본 문서로 돌아갈 수 있는 링크**는 매크로 본문(마크다운) 맨 위에
`*원본 문서: [제목](url)*` 형태의 마크다운 링크 문법으로 포함돼 있고,
그 바로 아래에 수정본 내용이 이어집니다 - 그래서 붙여넣으면 새로 만든
페이지 본문 맨 위에 원본 문서 링크가 나오고 바로 이어서 수정본 내용이
시작됩니다. 이 링크의 base URL은 REST API 호출에 쓰는
...
**왜 우리가 직접 Confluence storage format(XHTML)으로 변환하지 않고
Markdown 매크로에 맡기는가**: 예전에는 LLM이 만든 평문을 파이썬 코드로
직접 `<h1>`/`<ul>`/`<table>` 같은 XHTML 태그로 기계적으로 변환했는데,
표나 중첩 목록처럼 구조가 복잡해지면 실제 붙여넣기 결과가 원본과 다르게
재현되는 경우가 계속 나왔습니다(예: 번호 목록 사이에 표가 끼면 번호가
깨지는 문제). 지금은 변환을 직접 하지 않고, LLM이 만든 평문 마크다운을
그대로 Confluence의 Markdown 매크로에 감싸서 넘깁니다 - 실제 렌더링은
Confluence 자신의 마크다운 파서가 담당하므로, 우리가 문법을 하나하나
재현하다가 놓치는 예외 상황이 원천적으로 없어집니다. 이 매크로는 사내
Confluence에 설치돼 있는 것으로 확인됐지만, 다른 환경에서는 별도
애드온(예: "Markdown Macro for Confluence")이 필요할 수 있습니다 - 매크로가
없는 인스턴스에 붙여넣으면 "Unknown macro" 경고가 뜹니다.

**블록 사이 줄바꿈/빈 줄 보정**: 문서 전체 흐름을 감안해서 두 가지를
자동으로 보정합니다.
1. 빈 줄로 안 나뉜 문단 안의 순수 개행 하나는 표준 마크다운에서 줄바꿈이
   아니라 그냥 공백으로 접힙니다(소프트 브레이크). 그래서 그런 줄 끝에는
   마크다운 하드 브레이크 문법(줄 끝 `\`)을 자동으로 붙입니다.
2. 제목/문단/목록/표처럼 종류가 다른 블록이 빈 줄 없이 바로 붙어 있으면
   그 사이에 빈 줄을 넣습니다 - 대부분의 마크다운 파서(Confluence의
   Markdown 매크로 포함)는 표나 목록이라도 앞에 빈 줄이 없으면 바로 앞
   문단에 붙어 있는 것으로 보고 별도 블록으로 인식하지 못합니다(예: 문단
   바로 다음 줄에 표가 오면 표 전체가 그냥 문단 텍스트로 뭉개져 보입니다).

목록 항목끼리/표 행끼리처럼 같은 블록 안에서 자연스럽게 이어지는 줄과
들여쓰기된(중첩 목록 등) 줄은 건드리지 않습니다. LLM에게도 표에는
`|---|---|` 구분선을 꼭 넣고, 종류가 다른 블록 사이에는 빈 줄을 넣도록
프롬프트에 명시해 두지만(LLM이 놓칠 수 있어서), 최종적으로는 위 보정
로직이 한 번 더 안전망 역할을 합니다.
...
가이드라인 7개**는 이 도구가 준수 여부를 항목별로 표로 보여주고, **추가
권장 가이드 7개**는 리포트에 제안으로는 나오지만 이 표에는 포함되지 않습니다.
...
페이지마다 핵심 가이드라인 7개 각각의 준수 여부를 표로 보여줍니다. 하나의
점수(0~100점)로 뭉뚱그리지 않습니다 - 숫자 하나로는 어떤 항목이 실제로
문제인지 가려지기 쉽고, "몇 점이면 충분한지" 기준도 애매하기 때문입니다.

- **준수**: 해당 가이드라인을 확인할 방법(규칙 또는 LLM)이 있었고, 위반이 발견되지 않음
- **위반**: 규칙 또는 LLM이 위반 사례를 찾음
- **확인 불가**: LLM 전용 가이드라인인데 `LLM_BASE_URL`이 없거나 이번 분석에서 LLM 호출이 실패한 경우 (위반으로 간주하지 않음)
...
**최종 수정본이 있으면 "수정본 상태" 컬럼이 옆에 추가됩니다.** "최종
수정본 제안"(웹 UI)은 수정본을 만든 뒤 실제로 가이드라인을 지키는지
재검증까지 하므로(위 "만들고 끝내지 않고, 검증까지 합니다" 절 참고), 그
검증 결과를 원본 상태 옆에 나란히 보여줄 수 있습니다. 수정본은 평문이라
규칙 엔진이 못 돌고 LLM 재검증에만 의존하므로, 검증 자체가 실패하면(예:
일시적 네트워크 오류) 해당 항목은 "확인 불가"로 표시됩니다. CLI(`analyze_page`)
처럼 검증을 거치지 않는 경로에서는 이 컬럼 자체가 나오지 않습니다.
...
1. **새 문제 찾기**: 위 가이드라인 중 "LLM 전용"으로 표시된 항목들(대명사/
   ...
   ID를 함께 답하도록 프롬프트되어 있고, 이 값이 준수 표의 항목 판별에도
   그대로 쓰입니다.
...
준수 표에서 "확인 불가"로 표시됩니다). LLM이 새로 찾은 문제는 `llm-review`
...
수정본 생성 프롬프트도 1·2번(찾기 프롬프트)과 마찬가지로 **14개
가이드라인 전체 목록**을 알고 있습니다 - 그래서 규칙 엔진/찾기 단계가
이미 콕 집어 발견한 문제뿐 아니라, 목록에 없어도 가이드라인 기준으로
명백히 부족한 부분(예: 표에 그 표를 설명하는 캡션이 없음)이 있으면
함께 보강하도록 지시합니다. 다만 이미 가이드라인을 잘 지키고 있는
부분은 불필요하게 다시 쓰지 않습니다.

**문서 요약은 필요할 때만, 서론/결론은 억지로 만들지 않습니다**:
"문서의 자체 완결성"(core-7)과 "서론/본론/결론 흐름"(extra-1)
가이드라인 때문에 모든 문서에 억지로 서론/결론 절을 만들면 오히려
어색해집니다. 그래서 문서에 전체 내용을 한눈에 파악할 수 있는 요약/
개요가 전혀 없어서 맥락 파악이 어려운 경우에만, 문서 맨 앞이나 맨 뒤
중 더 자연스러운 쪽에 짧은 요약 문단을 추가하도록 지시합니다 - 이미
구조가 명확하거나 짧아서 요약이 필요 없는 문서라면 넣지 않습니다.

**만들고 끝내지 않고, 검증까지 합니다**: 프롬프트로 지시해도 자체
호스팅 소형 모델은 지시를 놓칠 수 있어서, "만들었다고 주장하는 것"과
"실제로 가이드라인을 지키는 것"은 다를 수 있습니다. 그래서 수정본을
만든 뒤, 그 수정본을 다시 찾기 프롬프트로 검토해서 가이드라인을 실제로
지키는지 확인합니다. 부족한 부분이 남아 있으면 그 문제들을 알려주고
다시 고치게 하는 과정을 최대 2회 반복합니다. 그래도 끝까지 못 고친
문제가 있으면(예: LLM이 문서 안 정보만으로는 판단할 수 없는 부분)
숨기지 않고 리포트에 `revision-unresolved` 규칙 ID로 "[수정본에도 남음]"
표시와 함께 그대로 남깁니다 - 사람이 확인해야 할 부분만 정확히 짚어주는
셈입니다. 이 검증/보정 과정은 LLM 호출을 추가로 늘리므로(최악의 경우
수정본 생성 1회 + 검증·보정 최대 4회 = 총 5회), 자체 호스팅 모델에서는
"최종 수정본 제안" 버튼을 누른 뒤 응답까지 시간이 더 걸릴 수 있습니다.
...
src/ai_friendly_doc/
  ...
  confluence_markdown_macro.py  # 평문 마크다운을 Confluence Markdown 매크로 XML로 감쌈
  parser.py             # storage format(XHTML) -> ParsedDoc 변환
  guidelines.py         # 14개 AI-friendly 가이드라인 정의 + 핵심 가이드라인 준수 판별
  analyzer.py           # 페이지 하나를 파싱 + 규칙(+ 설정 시 LLM) 적용 + 준수 판별
