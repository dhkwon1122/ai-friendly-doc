...
# app.py는 import 시점에 SESSION_SECRET이 없으면 바로 RuntimeError를 낸다.
# monkeypatch 픽스처는 아직 없는 시점(모듈 import 시점)이라 직접 세팅한다.
os.environ.setdefault("SESSION_SECRET", "test-secret-for-pytest")

from ai_friendly_doc.web.app import _fixed_base_url


def test_fixed_base_url_none_when_unset(monkeypatch):
    monkeypatch.delenv("CONFLUENCE_BASE_URL", raising=False)
    assert _fixed_base_url() is None
...
def test_fixed_base_url_none_when_blank(monkeypatch):
